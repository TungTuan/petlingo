...
export async function adminMiniGameWordsRoutes(app: FastifyInstance) {
  app.addHook("preHandler", verifyAuth);
  app.addHook("preHandler", requireAdmin);

  app.patch<{ Params: { id: string } }>("/:id", async (request, reply) => {
    const input = updateMiniGameWordSchema.parse(request.body);
    const word = await updateWord(request.params.id, input);
    return reply.send({ word });
  });

  app.delete<{ Params: { id: string } }>("/:id", async (request, reply) => {
    await deleteWord(request.params.id);
